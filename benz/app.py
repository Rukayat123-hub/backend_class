...
app = FastAPI(title="Simple FastAPI App", version="1.1.0")
...
@app.patch("/update-data/{id}", description="Update an existing entry by its index (partial update)")
def update_data(id: int, req: Item):
    if 0 <= id < len(data):
        # Only update fields provided in the request
        existing = data[id]
        updated = req.dict(exclude_unset=True)
...
@app.delete("/delete-data/{id}", description="Delete an entry by its index")
def delete_data(id: int):
    if 0 <= id < len(data):
        removed = data.pop(id)
        return {"message": f"🗑️ Data at index {id} deleted successfully", "deleted": removed, "data": data}
    return {"error": " Invalid ID — no data found at that index."}
